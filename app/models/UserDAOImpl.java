...
public class UserDAOImpl implements UserDAO {
    ...
    public User getUser(Long ID){
        ...
        if (user == null) {
            ...
            Alumni a = new Alumni(user.getName(),user.getPassword(),user.getEmail(),user.getSchool(), user.getUniversity(), user.getOccupation(), user.getIndustry(), user.getEmployer());
            a.setId(user.getId());
    ...
    /**
     *adds user video link to db
     *@param ID of the user
     *@param link of the video assiciated with the user
     */
    public void addUserVideo(Long ID, String link){
        SqlUpdate addVideo = Ebean.createSqlUpdate("UPDATE user SET video_link = :link WHERE id =:id");
        addVideo.setParameter("id",ID);
        addVideo.setParameter("link",link);
        addVideo.execute();
    }
}
